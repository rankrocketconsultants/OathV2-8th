  ...
  return (
    <View style={{ flex: 1, backgroundColor: "transparent" }}>
      ...
      <SegmentSwap depKey={mode} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: t.spacing.lg, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
          {groupsForMode().map(([label, rows]) => (
            <Card key={label} padded style={{ marginBottom: t.spacing.lg }}>
              ...
              <View style={{ borderTopWidth: t.hairlineWidth, borderTopColor: t.palette.hairline }}>
                {rows.length === 0 ? (
                  <View style={{ paddingVertical: t.spacing.md }}><Text style={{ color: t.palette.textTertiary }}>No items</Text></View>
                ) : (
                  rows.map((it, idx) => (
                    <View key={it.id}>
                      <TaskRow item={it} onToggle={(i) => updateItem(String(i.id), { completed: !i.completed })} onDelete={(i) => deleteItem(String(i.id))} showStatus={false} />
