...
type Palette = {
  ...
  textTertiary: string;
  accent: string;
  accent2: string;   // keep equal to accent for gradients
  onAccent: string;
  hairline: number;
  ...
  backdrop: string;
  timeChipBg: string;
  // optional status colors already used elsewhere
...
export const tokens: Record<Mode, Palette> = {
  light: {
    ...
    glass: "rgba(255,255,255,0.70)",          // ↑ stronger separation
  ...
  dark: {
    ...
    onAccent: "#001B14",
    hairline: 1,
    hairlineColor: "#262B30",                // ↑ brighter hairline for crispness
    glass: "rgba(21,24,26,0.78)",            // ↑ stronger tint under blur
