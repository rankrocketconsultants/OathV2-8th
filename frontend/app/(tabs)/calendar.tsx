...
export default function CalendarScreen() {
  ...
  const itemsForDay = useMemo(() => (d: Date) => {
  ...
  const openDay = (d: Date) => { setSelected(startOfDay(d)); setSheetOpen(true); };
    ...
    return (
      <Pressable onPress={() => openDay(d)} style={{ width: "14.2857%", paddingVertical: 8, paddingHorizontal: 6 }}>
        <View style={{
          ...
          <Text style={{ color: muted ? t.palette.textTertiary : t.palette.textPrimary, fontWeight: "700" }}>{d.getDate()}</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "transparent" }}>
      ...
      <SegmentSwap depKey={mode} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: t.spacing.lg, paddingBottom: bottomPad }} showsVerticalScrollIndicator={false}>
          {mode === "Week" && (
            <Card padded elevated>
              <View style={{ flexDirection: "row" }}>
                {weekStrip.map((d) => (
                  <Pressable key={d.toISOString()} onPress={() => setSelected(startOfDay(d))} onLongPress={() => openDay(d)} style={{ width: "14.2857%", alignItems: "center", paddingVertical: 10 }}>
