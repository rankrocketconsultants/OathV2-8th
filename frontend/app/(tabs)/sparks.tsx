...
import SegmentSwap from "../../src/components/SegmentSwap";
...
export default function SparksScreen() {
  ...
  const flat: FlatRow[] = useMemo(() => {
    ...
    if (seg === "Type") {
      const map = new Map<string, string[]>(); for (const s of sparks) { const k = (s.category || "General").trim(); map.set(k, [...(map.get(k) || []), s.id]); }
      const headers = Array.from(map.keys()).sort((a,b) => a.localeCompare(b));
      const rows: FlatRow[] = []; for (const h of headers) { rows.push({ kind: "header", key: `hdr-${h}`, title: h }); for (const id of map.get(h)!) rows.push({ kind: "row", key: id, id }); }
      return rows;
    }
    const cmap = new Map<string, string[]>(); for (const s of sparks) { const tokens = tokenize(s.title); const key = tokens[0] || s.category || "General"; const label = key[0].toUpperCase() + key.slice(1); cmap.set(label, [...(cmap.get(label) || []), s.id]); }
    const headers = Array.from(cmap.keys()).sort((a,b) => a.localeCompare(b));
    const rows: FlatRow[] = []; for (const h of headers) { const ids = cmap.get(h)!; rows.push({ kind: "header", key: `hdr-${h}`, title: h, actions: "convertAll", ids }); for (const id of ids) rows.push({ kind: "row", key: id, id }); }
    return rows;
  }, [sparks, seg]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "transparent" }}>
      ...
      {/* Animate content swap when segment changes */}
      <SegmentSwap depKey={seg.toString()} style={{ flex: 1, paddingHorizontal: t.spacing.lg }}>
        <GroupedList<FlatRow>
          data={flat}
                  ...
                  right={item.actions === "convertAll" ? (
                    <Pressable
                      ...
                      style={{
                        ...
                        borderWidth: t.hairlineWidth, borderColor: t.palette.accent
                      }}
                    >
